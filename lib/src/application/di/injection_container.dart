...
// Firebase & Auth
  ...
  injector.registerLazySingleton<OrderRepository>(
      () => OrderRepositoryImpl(injector<ApiService>()));
  injector.registerFactory(() => OrderCubit(injector(), injector()));
  injector.registerFactory(
      () => ProductListCubit(injector(), injector<OrderCubit>(), injector()));

  Hive.registerAdapter(OderedProductAdapter());
  Hive.registerAdapter(OrderDetailsModelAdapter());

  final orderBox = await Hive.openBox<OrderDetailsModel>('ordersBox');
  injector.registerSingleton<Box<OrderDetailsModel>>(orderBox);

  injector.registerLazySingleton<OrderRepositoryLocal>(
    () => OrderRepositoryLocalImpl(injector<Box<OrderDetailsModel>>()),
  );

  injector.registerFactory<OrderLocalCubit>(
    () => OrderLocalCubit(injector<OrderRepositoryLocal>()),
  );
}
